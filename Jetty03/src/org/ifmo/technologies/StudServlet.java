...
public class StudServlet extends HttpServlet {
	...
	private static final String STUD_TABLE = 
			...
			"  <col style=\'width: 20%%\'/><col style=\'width: 60%%\'/><col/>\n" +
			"<thead>" +
			"  <th>Зачетка</th><th>Имя</th><th>Группа</th>" +
			"</thead>" +
			"<tbody>" +
			"  %s" +
			"</tbody>" +
			"</table>\n";
	private static final String STUD_INFO =
			"<title>Информация о студенте</title>\n" +
			"<h2>Студент %s (%s)</h2>\n" +
			"<p>Группа: %s</p>\n";
	private static final String STUD_ROW =
			"<tr><td>%s</td><td>%s</td><td>%s</td></tr>\n";
	private static final String STUD_MARKS =
			"<p>%s: %s</p>\n";
	...
	private static final String ERR_ID_PAR_MISSED = "\'id\' параметр не задан";
    ...
    protected void doGet(
    	...
    	try {
    		if (path != null && !"/".equals(path)) {
    			// Ищем студента с заданным номером зачетки
     ...
     * Формат запроса:
     * POST /students?id=<номер зачетки>&name=<имя студента>&group=<номер группы>
     */
    protected void doPost(
    		HttpServletRequest request,
    		HttpServletResponse response) {
    	String path = request.getPathInfo();
    	String id = request.getParameter("id");		// Имя студента
    	...
    	try {
    		if (path != null && !"/".equals(path)) {
    			...
    			// Параметр 'id' не найден
    			error(response, Error.BAD_REQUEST, ERR_ID_PAR_MISSED);
    		} else if (name == null) {
    			...
    			students.put(name, new Student(id, name, group));
    		}
    	} catch (IOException e) {
    	}
    }
    ...
    @Override
    /**
     * Обработка запроса PUT - изменение группы и/или имени у существующего студента
     * Формат запроса:
     * PUT /students/<номер зачетки>?name=<имя студента>&group=<номер группы>
     */
    protected void doPut(
    		HttpServletRequest request,
    		HttpServletResponse response) {
    	String path = request.getPathInfo();
    	String name = request.getParameter("name");	// Номер группы
    	String group = request.getParameter("group");	// Номер группы
		response.setContentType(CONTENT_TYPE);
    	try {
    		if (path == null) {
    			...
    			String id = path.substring(1);	// Имя студента
    			Student student = students.get(id);
    			if (student == null) {
    				// Студент с заданным именем не найден
        			error(response, Error.NOT_FOUND, String.format(ERR_STUDENT_NOT_FOUND, id));
    			} else {
    				// Все нормально, записываем информацию об имени и группе
    ...
    private String studList() {
    	...
    	for (Student student : students.values()) {
    		rows.append(String.format(STUD_ROW, student.getId(), student.getName(), student.getGroup()));
    	}
