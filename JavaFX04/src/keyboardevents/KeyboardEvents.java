...
public final class KeyboardEvents extends Application {
    @Override
    ...
    private static final class Keyboard {
        ...
        public Node createNode() {
            ...
            installEventHandlers(keyboardNode);
            return keyboardNode;
        }

        private void installEventHandlers(final Parent keyboardNode) {
            // handler for key pressed / released events not handled by
            // key nodes
            final EventHandler<KeyEvent> letterKeyEventHandler =
                    keyEvent -> {
            ...
            final EventHandler<KeyEvent> leftRightKeyEventHandler =
            		keyEvent -> {
                        final Node nextFocusedNode;
                        switch (keyEvent.getCode()) {
                            case LEFT:
                                nextFocusedNode = getPreviousNode(keyboardNode,
                                                                  (Node)keyEvent.getTarget());
                                keyEvent.consume();
                                break;

                            case RIGHT:
                                nextFocusedNode = getNextNode(keyboardNode,
                                                              (Node)keyEvent.getTarget());
                                keyEvent.consume();
                                break;
            ...
            keyboardNode.addEventHandler(KeyEvent.KEY_PRESSED, letterKeyEventHandler);
            keyboardNode.addEventHandler(KeyEvent.KEY_RELEASED, letterKeyEventHandler);
            keyboardNode.addEventHandler(KeyEvent.KEY_PRESSED, leftRightKeyEventHandler);
        }
        ...
        private static Node getNextNode(final Parent parent,
        	...
        	for (Node candidate : parent.getChildrenUnmodifiable()) {
        		if (nodeFound) return candidate;
        		if (candidate == node) nodeFound = true;
        	}
            return null;
        }
        ...
        private static Node getPreviousNode(final Parent parent,
            ...
            Node lastNode = null;
            for (Node candidate : parent.getChildrenUnmodifiable()) {
            	if (candidate == node) return lastNode;
            	lastNode = candidate;
            }
            return null;
        }
